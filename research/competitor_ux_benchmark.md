...
## Activation evidence and first value: current benchmark, August 2026

Amplitude defines activation as the percentage of new users who reach a product-specific milestone showing that they have experienced core value, rather than a generic signup or login. It recommends a defined activation window, cohort-specific calculation, a consistent event definition, behavioural comparisons with retained users, and continual refinement of the original milestone hypothesis.[49] Chameleon separates leading onboarding indicators, such as completion, time to value, and activation, from retention and churn as later validation. Its guidance emphasizes a granular onboarding journey, a combined event and user-property model, segmentation by stage or intent, qualitative feedback, and a quick experiment loop rather than superficial completion measures.[50] ProductLed similarly anchors activation to the first delivery of the product promise and advises tracking a specific core-function event, observing drop-off before and after it, and accepting suitable friction where it helps a user learn enough to reach first value.[51]

The ASaaSI adaptation should create one narrow first-value experiment, not an abstract activation score. It should require a named new-user cohort, an explicit product-promise event, a time window, the starting and activated counts, the observed friction, and one reversible intervention. It should calculate only the specified cohort’s activation rate, distinguish a friction observation from causal proof, and save the event definition, before count, intervention, expected cohort response, and dated review in the shared decision record.
