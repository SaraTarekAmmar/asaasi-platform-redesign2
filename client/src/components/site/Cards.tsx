...
export function PersonCard({ initials, name, role, copy, tag, action, href = "/connect" }: { initials: string; name: string; role: string; copy: string; tag: string; action?: string; href?: string }) {
  const { t } = useLocale();
  return <article className="person-card landing-editorial-card landing-editorial-card--person"><div className="person-top"><div className="avatar" aria-hidden="true">{initials}</div><SignalTag tone="soft">{tag}</SignalTag></div><h3>{name}</h3><p className="person-role">{role}</p><p>{copy}</p><div className="card-footer"><span>{t("Open to a focused intro", "متاح لمقدمة مركزة")}</span><Link href={href} className="button button-ghost button-small">{action ?? t("View profile", "شاهد الملف")} <ArrowUpRight size={14} /></Link></div></article>;
}
...
export function ToolCard({ title, copy, tag, duration, tone = "", href = "/tools", next }: { title: string; copy: string; tag: string; duration: string; tone?: string; href?: string; next?: string }) {
  const { t } = useLocale();
  return <Link href={href} className="tool-card landing-editorial-card landing-editorial-card--tool"><div className="tool-card-main"><SignalTag tone={tone}>{tag}</SignalTag><h3>{title}</h3><p>{copy}</p></div><div className="tool-meta"><span><Clock3 size={12} /> {duration}</span><span>{next ?? t("Start with the question", "ابدأ بالسؤال")} <ArrowUpRight size={13} /></span></div></Link>;
}
...
export function ResourceCard({ title, copy, tag, duration, featured = false, href = "/learn", badge }: { title: string; copy: string; tag: string; duration: string; featured?: boolean; href?: string; badge?: string }) {
  return <Link href={href} className={`resource-card landing-editorial-card landing-editorial-card--resource ${featured ? "featured" : ""}`}><div><SignalTag tone={featured ? "" : "soft"}>{tag}</SignalTag>{badge && <span className="resource-badge">{badge}</span>}<h3>{title}</h3><p>{copy}</p></div><div className="resource-foot"><span>{duration}</span><ArrowUpRight size={18} /></div></Link>;
}
