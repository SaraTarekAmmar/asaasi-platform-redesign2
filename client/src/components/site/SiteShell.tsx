...
export function PageIntro({ label, title, copy, note }: { label: string; title: ReactNode; copy: string; note?: ReactNode }) {
  return <section className="page-hero"><div className="container page-hero-grid"><div><SectionLabel light>{label}</SectionLabel><h1>{title}</h1><p>{copy}</p></div>{note && <div className="page-hero-note">{note}</div>}</div></section>;
}
