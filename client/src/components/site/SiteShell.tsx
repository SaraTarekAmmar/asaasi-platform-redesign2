...
export function PageIntro({ label, title, copy, note }: { label: string; title: ReactNode; copy: string; note?: ReactNode }) {
  ...
  return <section className={`page-hero landing-page-intro landing-page-intro--${route}`}><div className="container page-hero-grid landing-page-intro__inner"><span className="landing-route-mark" aria-hidden="true"><i />{signal(routeMark[route] ?? "ASaaSI / MENA")}</span><div className="landing-page-intro__content"><SectionLabel light>{label}</SectionLabel><h1>{title}</h1><p>{copy}</p></div>{note && <div className="page-hero-note landing-page-intro__note">{note}</div>}<div className="landing-page-intro__trace" aria-hidden="true">{trace.map((item, index) => <span key={item} className={index === 1 ? "is-active" : ""}><i>{String(index + 1).padStart(2, "0")}</i>{signal(item)}</span>)}</div></div></section>;
}
