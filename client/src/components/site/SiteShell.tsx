...
export function PageIntro({ label, title, copy, note }: { label: string; title: ReactNode; copy: string; note?: ReactNode }) {
  ...
  const routeTrace: Record<string, [string, string, string]> = { community: ["QUESTION", "REPLY", "MOMENTUM"], learn: ["STAGE", "FIELD NOTE", "MOVE"], events: ["DATE", "ROOM", "FOLLOW-UP"], tools: ["INPUT", "WORK", "OUTPUT"], pricing: ["BUYER", "VALUE", "PRICE"], membership: ["VISITOR", "REGISTERED", "SUBSCRIBER"], providers: ["PRACTICE", "CONTEXT", "INTRO"], support: ["ROUTE", "CONTEXT", "ANSWER"], about: ["PEOPLE", "WORK", "MOMENTUM"], "operating-model": ["PILLARS", "PATHS", "EVIDENCE"], sponsors: ["AUDIENCE", "OUTCOME", "PLACEMENT"], applications: ["CONTEXT", "ROUTE", "REPLY"], "demo-day": ["PROGRESS", "ROOM", "CONVERSATION"], webinars: ["TOPIC", "LIVE ROOM", "FIELD NOTE"], workshops: ["INPUT", "WORKING SESSION", "DELIVERABLE"], contact: ["BLOCKER", "CONTEXT", "ANSWER"], terms: ["PROMISE", "BOUNDARY", "CONTRIBUTION"], accessibility: ["ACCESS", "ADAPT", "PARTICIPATE"], podcasts: ["OPERATOR", "CONTEXT", "TAKEAWAY"], courses: ["STAGE", "PRACTICE", "REPEAT"], playbooks: ["QUESTION", "SEQUENCE", "TEST"], videos: ["WATCH", "NOTE", "APPLY"], stories: ["FIELD NOTE", "PATTERN", "MOVE"], members: ["PERSON", "CONTEXT", "INTRO"], requests: ["ASK", "CONTEXT", "RESPONSE"] };
  ...
  return <motion.section initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.46, ease: [0.23, 1, 0.32, 1] }} className={`page-hero landing-page-intro landing-page-intro--${route}`}><div className="container page-hero-grid landing-page-intro__inner"><span className="landing-route-mark" aria-hidden="true"><i />{signal(routeMark[route] ?? "ASaaSI / MENA")}</span><motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.08, duration: 0.4 }} className="landing-page-intro__content"><SectionLabel light>{label}</SectionLabel><h1>{title}</h1><p>{copy}</p></motion.div>{note && <motion.div initial={{ opacity: 0, x: 12 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.14, duration: 0.42 }} className="page-hero-note landing-page-intro__note">{note}</motion.div>}<div className="landing-page-intro__trace" aria-hidden="true">{trace.map((item, index) => <motion.span initial={{ opacity: 0, y: 7 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.18 + index * 0.07, duration: 0.32 }} key={item} className={index === 1 ? "is-active" : ""}><i>{String(index + 1).padStart(2, "0")}</i>{signal(item)}</motion.span>)}</div></div></motion.section>;
}
