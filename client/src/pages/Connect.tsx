  ...
  return <>
    <section className="connect-landing-hero">
      ...
      <div className="connect-landing-visual">
        ...
        {/* ponytail: dropped the floating card's own paragraph + "Make the ask useful" CTA - it
            restated the hero's own point in different words and duplicated the hero's primary
            CTA to the same /requests/new destination. Left as a plain caption, no second ask. */}
        <div className="connect-floating-context"><span className="mono">{t("MATCHING / CONTEXT FIRST", "المطابقة / السياق أولا")}</span><div><img src={signalArtwork} alt="" /><p>{t("Start with the decision in front of you, not the largest directory.", "ابدأ بالقرار الذي أمامك، لا بأكبر دليل.")}</p></div></div>
      </div>
    </section>

    <section className="connect-landing-slate" id="match-slate"><div className="container"><div className="connect-slate-top"><div><SectionLabel>{t("People with context", "أشخاص يملكون السياق")}</SectionLabel><h2>{t("Not more contacts. Better starting points.", "ليس مزيدا من جهات الاتصال. بل نقاط بداية أفضل.")}</h2></div><div className="connect-slate-search"><Search size={16} aria-hidden="true" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={t("Search a person, market, or problem", "ابحث عن شخص أو سوق أو مشكلة")} aria-label={t("Search a person, market, or problem", "ابحث عن شخص أو سوق أو مشكلة")} /></div></div>{filtered.length ? <div className="connect-landing-grid">{filtered.map((person, index) => <article className={index === 0 ? "connect-landing-card connect-landing-card--lead" : "connect-landing-card"} key={person.id}><div className="connect-card-top"><span className="connect-card-index">{formatNum(index + 1)}</span><SignalTag tone="soft">{person.tag}</SignalTag></div><div className="connect-card-identity"><span>{person.initials}</span><div><h3>{person.name}</h3><p><MapPin size={12} /> {person.role}</p></div></div><p className="connect-card-reason">{person.copy}</p><div className="connect-card-bottom">{/* ponytail: this card's person.id used to link to /members/:id, but that route reads from
