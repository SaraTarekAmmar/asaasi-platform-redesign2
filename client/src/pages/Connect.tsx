  ...
  return <>
    <section className="connect-landing-hero">
      <div className="container connect-landing-hero__copy">
        ...
        <div className="connect-landing-hero__object" aria-label={t("Matching brief to introduction pathway", "مسار موجز المطابقة إلى المقدمة")}><span className="mono">{t("MATCHING OBJECT / ONE USEFUL ROUTE", "كائن المطابقة / مسار مفيد واحد")}</span><div><strong><i>{formatNum("01")}</i>{t("The decision", "القرار")}</strong><p>{t("Name the work that is blocked.", "سمِ العمل المتعثر.")}</p></div><div className="is-active"><strong><i>{formatNum("02")}</i>{t("The context", "السياق")}</strong><p>{t("Add stage, market, and evidence tried.", "أضف المرحلة والسوق والدليل الذي جُرّب.")}</p></div><div><strong><i>{formatNum("03")}</i>{t("The introduction", "المقدمة")}</strong><p>{t("Open one relevant conversation.", "افتح محادثة واحدة ذات صلة.")}</p></div></div>
        ...
        <div className="connect-landing-hero__rail" aria-label={t("How matching works", "كيف تعمل المطابقة")}><span><i />{formatNum("01")} {t("Name the decision", "سمِ القرار")}</span><span><i />{formatNum("02")} {t("Add the context", "أضف السياق")}</span><span><i />{formatNum("03")} {t("Meet the right person", "قابل الشخص المناسب")}</span></div>
      </div>
    ...
    <section className="connect-landing-intro"><div className="container"><div className="connect-intro-heading"><div><SectionLabel>{t("The founder network", "شبكة المؤسسين")}</SectionLabel><h2>{t("The right introduction begins before the profile card.", "المقدمة المناسبة تبدأ قبل بطاقة الملف الشخصي.")}</h2></div><p>{t("Good matching is not a keyword search. It starts with a real question, the conditions around it, and someone who has walked enough of the path to help.", "المطابقة الجيدة ليست بحثا بالكلمات المفتاحية. تبدأ بسؤال حقيقي والشروط المحيطة به وشخص سار في المسار بما يكفي للمساعدة.")}</p></div><div className="connect-landing-principles"><article><span>{formatNum("01")}</span><h3>{t("Name the work", "سمِ العمل")}</h3><p>{t("What has stalled, changed, or become expensive?", "ما الذي توقف أو تغير أو أصبح مكلفا؟")}</p></article><article className="is-warm"><span>{formatNum("02")}</span><h3>{t("Add context", "أضف السياق")}</h3><p>{t("Stage, market, team, and what has already been tried.", "المرحلة والسوق والفريق وما جُرّب بالفعل.")}</p></article><article><span>{formatNum("03")}</span><h3>{t("Open one route", "افتح مسارا واحدا")}</h3><p>{t("One relevant person and a reason to start the conversation.", "شخص مناسب وسبب لبدء المحادثة.")}</p></article></div></div></section>
