...
export default function Community() {
  ...
  return <>
    <PageIntro label={t("Community", "المجتمع")} title={t("Ask the question you actually need answered.", "اطرح السؤال الذي تحتاج إجابته فعلا.")} copy={t("Real questions from founders building right now, with real replies, not just likes.", "أسئلة حقيقية من مؤسسين يبنون الآن، بردود حقيقية، لا مجرد إعجابات.")} note={<div className="community-hero-thread-stack"><span className="mono">{t("THREAD / REPLY / ANSWER", "موضوع / رد / إجابة")}</span><ol><li className="is-active"><i>{formatNum("01")}</i><div><strong>{t("Post", "انشر")}</strong><small>{t("one focused question", "سؤالا مركزا واحدا")}</small></div></li><li><i>{formatNum("02")}</i><div><strong>{t("Reply", "رد")}</strong><small>{t("from relevant context", "من سياق ذي صلة")}</small></div></li><li><i>{formatNum("03")}</i><div><strong>{t("Answer", "إجابة")}</strong><small>{t("a useful next move", "خطوة تالية مفيدة")}</small></div></li></ol><a href="#ask" className="text-link">{t("Open the question desk", "افتح مكتب الأسئلة")} {isRTL ? <ArrowLeft size={14} /> : <ArrowRight size={14} />}</a></div>} />
