...
import { getFounderProfile, founderInitials } from "../lib/founderProfile";
...
export default function Community() {
  ...
  return <>
    ...
    {/* ponytail: swapped the bordered-box, corner-numbered connection-route for the shared
        ds-step-rail (thin baseline + circular index nodes) — same three-step content, no
        card shape pretending each step is a distinct product. */}
    <section className="section section-network-map connection-section"><div className="container"><div className="connection-route__intro"><div><SectionLabel>{t("How it works", "كيف يعمل")}</SectionLabel><h2>{t("Post it, and the network replies.", "انشره، وسترد الشبكة.")}</h2></div><p>{t("Ask a focused question, open a thread to see the full discussion, then comment with what you know.", "اطرح سؤالا مركزا، افتح موضوعا لرؤية النقاش كاملا، ثم علق بما تعرفه.")}</p></div><div className="ds-step-rail" role="list" aria-label={t("How community posts work", "كيف تعمل منشورات المجتمع")} style={{ "--ds-steps": 3 } as React.CSSProperties}><div role="listitem" data-step={formatNum("01")}><span className="ds-kicker">{t("START WITH / THE QUESTION", "ابدأ بـ / السؤال")}</span><h4>{t("Post", "انشر")}</h4><p>{t("Ask a focused question or share a result.", "اطرح سؤالا أو شارك نتيجة.")}</p></div><div role="listitem" data-step={formatNum("02")}><span className="ds-kicker">{t("OPEN / THE THREAD", "افتح / الموضوع")}</span><h4>{t("Open", "افتح")}</h4><p>{t("See the full discussion and who replied.", "شاهد النقاش كاملا ومن رد.")}</p></div><div role="listitem" data-step={formatNum("03")}><span className="ds-kicker">{t("LEAVE WITH / AN ANSWER", "غادر بـ / إجابة")}</span><h4>{t("Comment", "علق")}</h4><p>{t("Replies turn into a real answer.", "تتحول الردود إلى إجابة حقيقية.")}</p></div></div><div className="connection-route__footer"><div><span className="mono">{t("THE RULE OF A GOOD POST", "قاعدة المنشور الجيد")}</span><p>{t("Specific beats broad. One real question gets a real answer.", "التحديد يتفوق على العموم. سؤال حقيقي واحد يحصل على إجابة حقيقية.")}</p></div><a href="#ask" className="button button-dark button-small">{t("Ask a question", "اطرح سؤالا")} {isRTL ? <ArrowLeft size={14} /> : <ArrowRight size={14} />}</a></div></div></section>
    ...
    <section className="section"><div className="container content-layout"><aside className="filter-rail"><h3>{t("Filter by topic", "تصفية حسب الموضوع")}</h3><div className="filter-group"><div className="filter-options">{postTopics.map((option) => <label className="filter-option" key={option}><span><input type="radio" name="community-topic" checked={topic === option} onChange={() => setTopic(option)} /> {t(option, topicArabic[option])}</span><span className="filter-count">{formatNum(option === "All posts" ? allPosts.length : allPosts.filter((post) => post.topic === option).length)}</span></label>)}</div></div></aside><div><div className="results-head"><div><SectionLabel>{t("Browse posts", "تصفح المنشورات")}</SectionLabel><h2>{formatNum(filteredPosts.length)} {t(filteredPosts.length === 1 ? "post" : "posts", filteredPosts.length === 1 ? "منشور" : "منشورات")}</h2><p>{t("Every post has a real question and a founder waiting on a useful answer.", "كل منشور فيه سؤال حقيقي ومؤسس ينتظر إجابة مفيدة.")}</p></div><label style={{ position: "relative" }}><Search size={16} aria-hidden="true" style={{ position: "absolute", left: 15, top: 13, color: "var(--muted)" }} /><input className="search-input" style={{ paddingLeft: 40 }} value={query} onChange={(event) => setQuery(event.target.value)} placeholder={t("Search by title or author", "ابحث بالعنوان أو الكاتب")} aria-label={t("Search by title or author", "ابحث بالعنوان أو الكاتب")} /></label></div><div className="post-list">{displayPosts.map((post) => <Link href={`/community/${post.id}`} className="post-card" key={post.id}>
  ...
  return <>
    ...
    <section className="section"><div className="container">
      <article className="post-detail-card">
        ...
        <form className="post-detail-composer" onSubmit={submitComment}><div className="avatar avatar-yellow avatar-sm" aria-hidden="true">{founderInitials(getFounderProfile().name)}</div><input id="comment-input" value={comment} onChange={(event) => setComment(event.target.value)} placeholder={t("Write a comment...", "اكتب تعليقا...")} aria-label={t("Add a comment", "أضف تعليقا")} /><button type="submit" aria-label={t("Post comment", "انشر التعليق")}><Send size={15} /></button></form>
      </article>
    </div></section>
    {message && <Toast message={message} onClose={clearToast} />}
  </>;
