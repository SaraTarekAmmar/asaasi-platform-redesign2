...
export default function Pricing() {
  ...
  return <>
    ...
    <section className="section pricing-decision-section"><div className="container"><div className="section-header pricing-decision-heading"><div><SectionLabel>{t("Membership is a decision path", "العضوية مسار قرار")}</SectionLabel><h2>{t("Choose the constraint you need to remove next.", "اختر القيد الذي تحتاج إلى إزالته أولا.")}</h2></div><p>{t("Start with access. Upgrade only when a real limit is slowing the work in front of you.", "ابدأ بالوصول. رقِ فقط عندما يبطئ حد حقيقي العمل الذي أمامك.")}</p></div><ol className="pricing-decision-map">{[
      { trigger: t("I need context before I commit.", "أحتاج إلى سياق قبل أن ألتزم."), unlock: t("See the ecosystem and make your first useful ask.", "شاهد المنظومة وقدم طلبك المفيد الأول."), plan: t("Free / Access", "مجاني / وصول") },
      { trigger: t("I need more introductions and a fuller playbook.", "أحتاج إلى مقدمات أكثر ودليل عمل أوسع."), unlock: t("Move with the library, events, and the people already doing the work.", "تحرك مع المكتبة والفعاليات والأشخاص الذين يؤدون العمل بالفعل."), plan: t("Member / Momentum", "عضو / زخم") },
      { trigger: t("I need guided leverage beyond solo mode.", "أحتاج إلى دعم موجّه يتجاوز العمل الفردي."), unlock: t("Add mentors, masterminds, market context, and team access.", "أضف مرشدين ومجموعات إرشادية وسياق السوق ووصول الفريق."), plan: t("Growth / Scale", "نمو / توسع") },
    ].map((step, index) => <li key={step.plan} className={index === 1 ? "is-current" : ""}><span className="pricing-decision-node">{formatNum(index + 1)}</span><div><span className="mono">{t("THE CONSTRAINT", "القيد")}</span><h3>{step.trigger}</h3></div><div><span className="mono">{t("WHAT OPENS", "ما الذي ينفتح")}</span><p>{step.unlock}</p></div><strong>{step.plan}</strong></li>)}</ol></div></section>
